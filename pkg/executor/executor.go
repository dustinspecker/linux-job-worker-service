...
// Run executes the provided command with the provided arguments.
// Run will do the following:
// 1. Mount a new proc filesystem so that commands like `ps -ef` only show the processes in the pid namespace.
// 2. Run the provided command with the provided arguments.
// 3. Wait for the command to finish or for a SIGTERM signal to be received.
// 4. If the command exits with children processes left behind, then wait for a SIGTERM signal to terminate children processes.
// 5. If a SIGTERM signal is received, then terminate children processes.
// 6. Unmount the proc filesystem.
